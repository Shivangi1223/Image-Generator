...
const Login = () => {
  ...
  const { setShowLogin, setToken, setUser, setCredit } = useContext(AppContext)
    ...
    try {
      if (state === "Login") {
        ...
        if (data.success) {
          ...
          localStorage.setItem("token", data.token)
          setShowLogin(false)
          toast.success("Account created successfully! You received 400 free credits!")
        } else {
          toast.error(data.message)
        }
      }
    ...
    <div className="fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center">
      ...
      >
        ...
        <div className="border px-6 py-2 flex items-center gap-2 rounded-full mt-4">
          <img src={assets.email_icon || "/placeholder.svg"} alt="" />
          <input
        ...
        <div className="border px-6 py-2 flex items-center gap-2 rounded-full mt-4">
          <img src={assets.lock_icon || "/placeholder.svg"} alt="" />
          <input
